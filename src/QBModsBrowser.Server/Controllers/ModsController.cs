...
public class ModsController : ControllerBase
{
        ...
        [FromQuery] int pageSize = 24)
    {
        ...
        // Lightweight assumed download flag (no HTTP calls - cache or heuristic only)
        foreach (var e in enriched)
        {
            if (e.HasDirectDownload) continue;
            if (_assumed.HasCachedCandidates(e.TopicId))
            {
                var cached = _assumed.GetCachedCandidates(e.TopicId);
                e.HasAssumedDownload = cached != null && cached.Count > 0;
                e.AssumedDownloadCount = cached?.Count ?? 0;
    ...
    public async Task<IActionResult> GetAssumedDownloads(int id)
    {
        ...
        if (context.EnrichedSummary != null && !context.EnrichedSummary.HasDirectDownload && context.Detail.Links.Count > 0)
        {
            try
            {
